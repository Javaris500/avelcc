...
**Two package-hash definitions exist in the `exports` table, and the rows do not say which is which.** `444fa0cd5822e6cb…` is the OLD definition, computed over every rendered entry including `manifest.json`; `0b094e20b37481e9…` is the correct one, which excludes it because a manifest cannot hash itself. The renderer never moved — the same bytes were asked a different question. Four rows predate the fix, and the archive endpoint permanently refuses them with `DETERMINISM_VIOLATION`, which is correct: a hash computed by different code cannot be shown to match a rebuild. **At rest those rows are indistinguishable from a genuine nondeterminism failure**, and one session already read them that way with the code in front of them. Anyone reading two disagreeing hashes should check this line before suspecting the render path.
...
5. ~~Export delivery targets.~~ **Built 2026-08-29** on the `export-engine` branch — three targets, the guards, the lifecycle and the routes. See `EXPORT-ENGINE.md` for how it works and why. Two things gate a real delivery, neither of them code in that engine: **no Connection can be created**, so no GitHub target can execute, and **nothing assembles a `RenderMission` from Neon**, so every mission currently renders the golden fixture package.
