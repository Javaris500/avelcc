...
| # | Module | State |
|---|---|---|
| 1 | Schema & validation (Zod) | 🟡 steps 0–2 built (9 core entities + joins); step 3's `Connection` · `Export` **in progress 2026-08-29**; step 4 (KnowledgeEntry · ActivityLog · User) absent; pgvector not enabled |
| 2 | Persistence (Drizzle + migrations) | 🟡 migrations `0000`–`0008` applied to Neon (`0008` = mission `status` default, nullable `cut`); runtime db client built (`modules/db/client.ts` — neon-http on the pooled `DATABASE_URL`, server-only). Entity coverage is the remaining gap, not the connection |
...
- **Migration step 3 — `Connection` and `Export` written, not yet applied.** `RepoPolicy` landed earlier the same day (migration `0006`, trigger `0007`), applied and verified live. `Connection` and `Export` are in `schema.ts` with migrations `0009`/`0010` generated and **awaiting a decision to push to Neon**. Until they are applied, export delivery targets cannot begin.
...
4. **Apply `0009`/`0010`, then reconcile the canonical doc.** The tables are written; pushing them is a decision, and the four Export/Connection rulings taken on 2026-08-29 need to land in `DATA-CONTRACTS-V2.md` or they will be re-litigated. Four further conflicts in those blocks are still open and unresolved.
