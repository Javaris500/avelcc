...
export function Shell({
  ...
  return (
    <div
      className={cn('app flex h-screen bg-app-bg text-text', theme === 'light' && 'light')}
      data-testid="app-shell"
      data-theme={theme}
    >
      <Sidebar onSignOut={onSignOut} session={session} />
      <div className="flex min-w-0 flex-1 flex-col">
        <TopBar breadcrumb={breadcrumb} onToggleTheme={onToggleTheme} theme={theme} />
        <main className="app-scroll min-h-0 flex-1 overflow-y-auto" data-testid="main">
          {children}
        </main>
      </div>
    </div>
