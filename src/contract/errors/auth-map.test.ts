...
import { AUTH_CODES } from '#/contract/shared/errors'
...
describe('auth error map', () => {
  ...
  it('stays a separate vocabulary from the export codes', () => {
    for (const a of AUTH_CODES) {
      expect(ERROR_CODES).not.toContain(a as unknown as (typeof ERROR_CODES)[number])
    }
  })
  ...
  it('never reveals which half of the credential was wrong', () => {
    ...
    expect(text).not.toMatch(/no account|unknown email|email not found|user not found/)
