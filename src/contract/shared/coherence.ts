...
export type CoherenceWarning =
  ...
  | { code: 'writable_overlap'; reason: string; agents: [string, string]; witness: string }
...
function unifySegment(a: string, b: string): string | null {
  ...
  for (const cand of [concreteSegment(a), concreteSegment(b)]) {
    if (picomatch.isMatch(cand, a, { dot: true }) && picomatch.isMatch(cand, b, { dot: true })) {
      return cand
    }
  }
  return null
...
function unify(a: string[], b: string[]): string[] | null {
  ...
  if (a[0] === '**') {
    for (let take = 0; take <= b.length; take++) {
      ...
      if (rest !== null) return [...b.slice(0, take).map(concreteSegment), ...rest]
    }
    return null
  }
  if (b[0] === '**') {
    for (let take = 0; take <= a.length; take++) {
      ...
      if (rest !== null) return [...a.slice(0, take).map(concreteSegment), ...rest]
    }
    return null
  }
...
export function computeCoherence(
  ...
  if (earliest === undefined) {
    block = {
      code: 'no_agents_in_first_wave',
      reason: 'The playbook declares no waves, so no earliest wave exists to staff.',
      wave: '',
    }
  ...
  if (horizontals.length === 0) {
    warnings.push({
      ...
      reason: 'No horizontal agent is assigned, so no role owns the seams between features.',
    })
  }
  if (earliest !== undefined && !active.some((a) => a.wave !== earliest && a.kind === 'horizontal')) {
    warnings.push({
      code: 'no_verification_role',
      reason: 'No horizontal agent is assigned after the earliest wave, so nothing verifies the features.',
    })
  }

  for (const h of [...horizontals].sort((a, b) => byCodepoint(a.slug, b.slug))) {
