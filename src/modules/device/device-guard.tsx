...
function useRouteDevice(): Device | undefined {
	...
	for (let i = matches.length - 1; i >= 0; i--) {
		const device = (matches[i]?.staticData as { device?: Device } | undefined)
			?.device;
...
function DesktopRequired() {
	...
	return (
		<div
			className="flex flex-col items-start gap-3 px-6 py-14"
			data-testid="desktop-required"
		>
			<span className="font-mono text-xs text-gate-stale">
				DESKTOP REQUIRED
			</span>
			...
			<p className="max-w-[52ch] text-sm leading-relaxed text-text-muted">
				{DEVICE_LABEL.construction}. Assembling a roster or starting a delivery
				means seeing three panels and a file list at once, and getting it wrong
				here is expensive. Reviewing and approving still works on a phone.
			</p>
			...
			<p
				className="pt-1 font-mono text-micro break-all text-text-subtle"
				data-testid="desktop-required-url"
			>
				{href}
			</p>
		</div>
