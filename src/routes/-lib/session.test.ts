...
import { OPERATOR_COOKIE, SESSION_COOKIE } from "#/routes/-lib/oauth";
...
describe("session cookie split", () => {
	...
	it("has the client gate reading the DISPLAY cookie, never the httpOnly one", () => {
		const src = readFileSync("src/routes/-lib/session.ts", "utf8");
	...
	it("marks only the trusted cookie httpOnly", async () => {
		const { cookie } = await import("#/routes/-lib/oauth");
		expect(cookie(SESSION_COOKIE, "x", { httpOnly: true })).toContain("HttpOnly");
