...
import { listAgentTemplates } from "#/modules/catalog/service";
import { db } from "#/modules/db/client";
import { shielded, withMethodGuard } from "#/modules/http/shielded";

/**
 * GET /api/agent-templates — every agent template, revoked included, with its skills.
 *
...
export const Route = createFileRoute("/api/agent-templates")({
	server: {
		handlers: withMethodGuard({
			GET: ({ request }) =>
				shielded("agent template list", async () => {
					...
					const { items, total, nextCursor } = await listAgentTemplates(
						db,
						query,
					);
