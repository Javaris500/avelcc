...
import { listSkillSources } from "#/modules/catalog/service";
import { db } from "#/modules/db/client";
import { shielded, withMethodGuard } from "#/modules/http/shielded";

/**
 * GET /api/skill-sources — every skill source, with live and revoked counts kept apart.
 *
 ...
 */
export const Route = createFileRoute("/api/skill-sources")({
	server: {
		handlers: withMethodGuard({
			GET: ({ request }) =>
				shielded("skill source list", async () => {
					...
					const { items, total, nextCursor } = await listSkillSources(
						db,
						query,
					);
